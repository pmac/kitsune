...
init: .docker-build-pull
    ${DC} run web ./manage.py migrate
    ${DC} run web yarn
    ${DC} run web ./node_modules/.bin/bower install --allow-root
...
.PHONY: default clean build build-full pull docs lint run djshell shell test test-image lint-image rebuild build-ci test-ci test-js-ci lint-ci
